...
// Fix 3: single source of truth for health → color
const healthBadgeColor = (status: string) => {
  const s = status.toUpperCase()
  return s === "CRITICAL" ? "bg-red-500/10 text-red-500 border-red-500/20"
    : s === "WARNING"  ? "bg-yellow-500/10 text-yellow-500 border-yellow-500/20"
    :                    "bg-green-500/10 text-green-500 border-green-500/20"
}
const healthTextColor = (status: string) => {
  const s = status.toUpperCase()
  return s === "CRITICAL" ? "text-red-500" : s === "WARNING" ? "text-yellow-500" : "text-green-500"
}
...
export function ClusterDashboard() {
  ...
  // Fix 1a: loading state — ov undefined = first paint, show placeholder
  if (ov === undefined) {
    return (
      <Card className="bg-card border-border">
        <CardContent className="py-6 text-center text-muted-foreground text-sm">Loading cluster…</CardContent>
      </Card>
    )
  }
  ...
  const worstColor = healthTextColor(worst)

  // Fix 1b: all-offline guard — no online nodes means aggregates are meaningless
  const hasOnline = onlineNodes.length > 0
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-card border-border">
        <CardContent className="py-4 flex flex-wrap items-center gap-x-8 gap-y-3">
          {hasOnline ? (
            ...
            <div className="text-sm text-muted-foreground">No nodes online</div>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {onlineNodes.map((n) => {
          ...
          const hColor = healthBadgeColor(hs)  // Fix 3: use shared helper
          const t = tempNum(n.system)           // Fix 2: compute once
          return (
            <Card key={n.node} className="bg-card border-border cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setExpanded({ node: n.node, isSelf: n.is_self })}>
              ...
              <CardContent className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <div className="flex items-center gap-1"><Thermometer className="h-3.5 w-3.5 text-muted-foreground" />{t || "—"}{t ? "°C" : ""}</div>
